...
// Mock window.hlx object for EDS components
if (!window.hlx) {
  window.hlx = {
    codeBasePath: '',
    lighthouse: false,
...
export function loadCSS(href) {
  return new Promise((resolve) => {
    // Normalize the href path for Storybook
    let normalizedHref = href;
    
    // If href contains codeBasePath, remove it as we're using Vite aliases
    if (href.includes('${window.hlx.codeBasePath}')) {
      normalizedHref = href.replace('${window.hlx.codeBasePath}', '');
    }
    
    // Convert absolute paths to relative for Vite
    if (normalizedHref.startsWith('/blocks/')) {
      normalizedHref = normalizedHref.replace('/blocks/', '../../../blocks/');
    }
    
    // Controlla se il CSS è già caricato
    const existingLink = document.querySelector(`link[href="${normalizedHref}"]`);
    if (existingLink) {
      resolve();
      return;
    }
